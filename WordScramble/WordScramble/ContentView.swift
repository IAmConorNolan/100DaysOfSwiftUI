...
struct ContentView: View {
                ...
                Section("Used Words") {
                    HStack {
                        Text("Score: ")
                            ...
                            .bold()
                        Spacer()
                        Text("\(usedWords.reduce(0, { $0 + $1.count}))")
                    }
    ...
    private func startGame() {
        guard let fileUrl = Bundle.main.url(forResource: "start", withExtension: "txt"),
              ...
              let randomWord = fileContents.components(separatedBy: .newlines).randomElement()
        else {
            fatalError("Failed to load starting words.")
        }
